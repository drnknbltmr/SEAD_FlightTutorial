...
l_h = x_ac_nose_tail - x_ac_nose_wing
...
# Fuselage correction
delta_fus_cruise   = 6.094 - CL_alpha_w_cruise_ref
delta_fus_approach = 4.710 - CL_alpha_w_approach_ref
...
rows = [
    ...
    ("CLα_wf cruise",           CL_alpha_wf_cruise_ref,    CL_alpha_wf_cruise_exx),
    ("CLα_wf approach",         CL_alpha_wf_approach_ref,  CL_alpha_wf_approach_exx),
    ("x_ac_nacelle",            x_ac_n_ref,                x_ac_n_exx),
    ("x_ac (a/c-less-tail)",    x_ac_ref,                  x_ac_exx),
...
for ax, (tag, p) in zip(axes, configs.items()):
    ...
    ax.plot(cg_n, Vh_arr, color=colors['neutral'], lw=1.4, ls='--', label='Neutral stability line')
    ax.plot(cg_s, Vh_arr, color=colors['stab'],    lw=2.0, label=f'Stability line (SM=5%)')
    ax.plot(cg_arr, Vh_c, color=colors['ctrl'],    lw=2.0, label='Controllability line')
    ...
    ax.axhline(Vh_design, color=colors['Vh'], lw=1.2, ls=':', label=f'Design V̄_H = {Vh_design:.3f}')
    ...
    if p['cg_fwd'] is not None and p['cg_aft'] is not None:
        ax.plot([p['cg_fwd'], p['cg_aft']], [Vh_design, Vh_design],
                ...
                label=f"CG range [{p['cg_fwd']*100:.1f}%–{p['cg_aft']*100:.1f}%]")
        ax.plot([p['cg_fwd'], p['cg_aft']], [Vh_design, Vh_design], 'v', color=colors['cg'], ms=7)

    if p['oew_cg'] is not None:
        ax.axvline(p['oew_cg'], color=colors['oew'], lw=1.2, ls=':', label=f"OEW CG = {p['oew_cg']*100:.1f}%")
